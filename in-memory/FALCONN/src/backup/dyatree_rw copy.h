...
private:
    static double prob_calc(int n, int x, int z){
        int bias = (z-n)/2 - (z < n);
        return exp(MarsagliaGenerator::_factable.lncomb(n, x) + MarsagliaGenerator::_factable.lncomb(n, z-x) 
                    - MarsagliaGenerator::_factable.lncomb(n, x-bias) + n * log2 - 
                    MarsagliaGenerator::_factable.lncomb(2*n, z)  ) / 1.5;
    }
    ...
    int rej_sample(int z, unsigned level, unsigned hash, unsigned seed) const{
        ...
        while(prob != 1. && hash > (unsigned)(prob * twop32)){
            if (prob >= 1 || prob == 0 )// || prob < 0.001)
                ...
                //std::cout << MarsagliaGenerator::_factable.lncomb(n, x) + MarsagliaGenerator::_factable.lncomb(n, z-x) 
                 //   - MarsagliaGenerator::_factable.lncomb(n, x-bias) + n * log2 - 
