...
constexpr int NUM_EXPERIMENTS = 100;
...
int main(int argc, char * argv[]) {
  ...
  MultiProbeCPDebug<CrossPolytopeHash2<>> multiprobe2(cp_hash, PROBES);
  ...
  for (int exp = 0; exp < NUM_EXPERIMENTS; ++exp) {
    ...
    multiprobe.setup_probing(mp_vec, L*PROBES);
    multiprobe2.setup_probing(std::move(mp_vec), L*PROBES);
    HashType pb1, pb2;
    int_fast32_t table1, table2;
    for (int probe = 0; probe < L*PROBES; ++probe) {
      multiprobe.get_next_probe(&pb1, &table1);
      multiprobe2.get_next_probe(&pb2, &table2);
      if (pb1 == pb2 && table1 == table2) continue;
      else {
        cout << "fail:" << seed << endl;
        exit(1);
      }
