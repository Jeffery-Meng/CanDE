#ifndef __NN_QUERYUSED_H__
#define __NN_QUERYUSED_H__
...
public:
  ...
  void cande_infer_associative(const QueryType& q,
    ...
    int num_values;
    switch (task) {
    case CanDETask::kKDE: num_values = falconn_config.gamma.size(); break;
    case CanDETask::kQDDE: num_values = num_bins; break;
    case CanDETask::kRecall: num_values = 1; break;
    default: throw NearestNeighborQueryError("Unknown CanDE task.");
    }
  ...
  void knn_and_qdde_infer(const QueryType& q,
    ...
    // std::ofstream fout(middle_filename);
    // for (auto& rhos_table : rhos_bin_table) {
    //   write_data(fout, rhos_table);
    // }
    //std::cout << full_count << std::endl;
  }

  // CP-adjusted weighted inference for knn recall,
  // implemented by push-and-sort
  void knn_and_weighted_recall(const QueryType& q,
                          int_fast64_t k, int_fast64_t num_probes,
                          int_fast64_t max_num_candidates,
                          std::vector<LSHTableKeyType>* result,
                          std::vector<CoordinateType>* result_infer) {
    ...
    int num_tables = falconn_config.hash_table_params[0].l;
    // Tables are represented by 32-bit bitmaps.
    assert(num_tables <= 32);
    // knn table is not that big
    assert(k <= 16384);
    ...
    initialize_heap();
    knn_tables_.clear();
    result_infer->assign(1, 0.0);

    // Step 2: compute all distances and record origin tables
    ...
    int table = 0, count = 0; // full_count = 0;
    while (iter.is_valid()) {
      bool is_in_knn_table;
      int key = iter.get_key();
      knn_tables_.find(key, is_in_knn_table);
    ...
    std::vector<int> uniq_cands_per_bin(1, 0);
    // candidates by table
    std::vector<std::vector<int>> tcands_per_bin(1, std::vector<int>(num_tables, 0));
    // # candidates unique to a given table
    auto utcands_per_bin = tcands_per_bin;
    ...
    for (auto key : candidates_) {
      ONIAK::HashInsertionStatus status = ONIAK::HashInsertionStatus::kNewlyInserted;
      float distance = 0; //distances[count];
      int bin = 0; // range.bin_translate(distance);
      while (count >= candidate_num_per_table[table]) ++table;
      auto& table_find = table_bin_ht_.find_or_insert(key, status);
      // ONIAK::TableBinPair table_bin = {2,2};
      if (status == ONIAK::HashInsertionStatus::kNewlyInserted) {
