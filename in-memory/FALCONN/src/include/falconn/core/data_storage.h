 ...
 public:
  class FullSequenceIterator {
   public:
    ...
    KeyType get_key() const { return index_; }
  ...
  class SubsequenceIterator {
   public:
    ...
    int get_next_key() const {
      if (index_ + 3 < static_cast<int_fast64_t>(keys_->size())) {
        return (*keys_)[index_ + 3]; 
      } else return -1;
    }
  ...
  int dimension() const { return data_[0].size(); }
...
/*
  WritableSequenceIterator get_writable_sequence() {
    return WritableSequenceIterator(*this);
  }*/
