...
public:
  ...
  typedef std::pair<ValueType, KeyType> VKpair;
  ...
  void add_entries_in_keys(const std::vector<KeyType>& hash_keys, const std::vector<ValueType>& keys) {
    // TODO for Huayi:
     // Add a part of the points to the hash table.
     // The points to add are given by keys
     // The hash key of all points are in hash_keys
     // please refer to the add_entries method above when you implement it
    if (num_buckets_ <= 0) {
      throw FlatHashTableError("Non-positive number of buckets");
    }
    if (entries_added_) {
    ...
    std::vector<VKpair> pairs(hash_keys.size());
    indices_.resize(hash_keys.size());

    for (IndexType ii = 0; static_cast<size_t>(ii) < indices_.size(); ++ii) {
      if (hash_keys[ii] >= static_cast<KeyType>(num_buckets_) || hash_keys[ii] < 0) {
        throw FlatHashTableError("Key value out of range.");
      }
      pairs[ii].first = keys[ii];
      pairs[ii].second = hash_keys[ii];
    }

    std::sort(pairs.begin(), pairs.end(), [](VKpair a, VKpair b) {return a.second < b.second;});
    ...
    for (KeyType bucket = 0; bucket < static_cast<KeyType>(num_buckets_);
      ...
      bucket_list_[bucket] = cur_index;
      IndexType end_index = cur_index;
      while (end_index < static_cast<IndexType>(hash_keys.size()) &&
        pairs[end_index].second == bucket) {
        indices_[end_index] = pairs[end_index].first;
        ++end_index;
      }
      cur_index = end_index;
    }
  ...
  void add_entries_from_stream(std::ifstream& fin) {
    ...
    bucket_list_.resize(num_buckets_ + 1, 0);
    fin.read(reinterpret_cast<char*> (bucket_list_.data()), (num_buckets_ + 1) * sizeof(IndexType));
    ...
    fin.read(reinterpret_cast<char*> (indices_.data()), index_num * sizeof(ValueType));
    // for (int i = 0; i < index_num; ++i){
    //   fin >> indices_[i];
    // }
    if (!fin || index_num != falconn_config.num_points) {
      throw FlatHashTableError("Error reading from file.");
    }
    for (int i = 0; i < num_buckets_; ++i) {
      if (bucket_list_[i] > bucket_list_[i + 1]) {
        throw FlatHashTableError("Error reading from file.");
      }
    }
  ...
  void dump_entries_to_stream(std::ofstream& fout) const {
    fout.write(reinterpret_cast<const char*> (bucket_list_.data()), (num_buckets_ + 1) * sizeof(IndexType));
    // for (int i = 0; i <= num_buckets_; ++i) {
    //   fout << bucket_list_[i] << "\t";
    // }
    // fout << std::endl;
    // bucket_list_[num_buckets_] == keys.size();
    fout.write(reinterpret_cast<const char*> (indices_.data()), indices_.size() * sizeof(ValueType));
