...
result_ends = ["precomputed","infer"] # "infer"
...
for j in range(len(datasets)):
    for i in range(len(modes)):
        ...
        conf["knn filename"] = os.path.join(exp_path,  "summary/knn_accu_" + result_ends[i]+ ".ivecs")
        conf["kde filename"] = os.path.join(exp_path,  "summary/kde_accu_" + result_ends[i]+ ".fvecs")
        conf["cande table width"] = 16
        conf["result filename"] = os.path.join(exp_path,  "knn_kde_accu_" + result_ends[i]+  ".txt")


        config_file = os.path.join(exp_path,  "config/knnkdei_final_accu_"+result_ends[i]+ ".json")
