...
exp_path = "/media/mydrive/distribution/ann-codes/in-memory/EXPERIMENTS/enronN"
...
gammas = [0.8, 1.0] # doubles every 10 values
gammas = [scott * x for x in gammas]
gammas.extend([50, 5.0, 1.0, 0.5])
# 20 gamma values, the last is 3 times the first
config["gamma"] =  gammas
config["tau"] = 1e-6
config["dataset"] = "enronN"
...
with open(os.path.join(exp_path, "ann_ps3.json")) as fin:
    conf2 = json.load(fin)
