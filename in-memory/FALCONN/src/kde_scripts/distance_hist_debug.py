...
dists = list(np.arange(lb, ub, 0.1))
probs = []
for i in dists:
    probs.append( mp_prob(i))
matplotlib.rcParams.update({'font.size': 12})
plt.hist(distance[query, :], range=(lb,ub), bins=50, alpha=0.7, label="ground truth")
plt.hist(cand_dist, bins=50, range=(lb, ub), weights=weights, alpha=0.7, color="r", label="estimated")
