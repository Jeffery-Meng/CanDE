...
recall_qh_prior = fvecs_read(exp_path + "/qh_recalls.fvecs")
    ...
    for bin_id, cand_idx in enumerate(cand_f.groups):
        ...
        for table in range(l):
            ...
            if len(cand_effective) == 0:
                continue
            denom = len(cand_effective) + Bay_weight
            numer = len(cand_effective & set(candiate_tables[table])) + Bay_weight * recall_qh_prior[bin_id * l + table]
