...
for query in range(nq):
    ...
    candidate_bins = candidate_bins / probs

    scale_ratio = n / np.sum(candidate_bins)
    candidate_bins *= scale_ratio
    
    plt.clf()
    matplotlib.rcParams.update({'font.size': 12})
    plt.hist(distance[query, :], range=PLOT_RANGE, bins=50, alpha=0.7, label="ground truth")
    plt.stairs(candidate_bins, edges, alpha=0.7, color="r", label="estimated", linewidth=3)
    ...
    file_path = exp_path / "plots" / "mnist-scale-q{}.pdf".format(query)
    plt.savefig(str(file_path))

    gt_histogram, edges = np.histogram(distance[query, :], bins=50, range=PLOT_RANGE)
    diff = np.abs(candidate_bins - gt_histogram, dtype=np.float64)
    if mre is None:
...
exp_path = pathlib.Path(sys.argv[1]).parent.parent 
file_path = exp_path / "plots" / "scale-mre.pdf"
plt.savefig(str(file_path))
