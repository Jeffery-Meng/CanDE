...
RS::RS(shared_ptr<MatrixXd> data, shared_ptr<Kernel> k, int samples) {
    ...
    //X = make_shared<MatrixXd>(samples, data);
    /*int i = 0;
...
std::vector<double> RS::MoM_precomputed(int qid, int L, int m, float gamma, const MatrixXf& distances) {
    std::random_device rd;  //Will be used to obtain a seed for the random number engine
    std::mt19937_64 rng = std::mt19937_64(rd());
    std::uniform_int_distribution<int> distribution(0, numPoints - 1);
    ...
    for (int i = 0; i < L; i ++) {
        ...
        for (int j = 0; j < m; j ++) {
            int idx = indices[j];
            if (m == numPoints) { idx = j; }
            Z[i] += kernel->density(distances(qid, idx) / gamma / sqrt2);
        }
    }
    return Z;
}