...
public:
    ...
    // query with precomputed distances
    double query_precomputed(int qid, double lb, int m, float gamma, const MatrixXf& distances) {
        int L = 1;
        auto t1 = std::chrono::high_resolution_clock::now();
        std::vector<double> Z = MoM_precomputed(qid, L, m, gamma, distances);
        double est = mathUtils::median(Z) / m;
        auto t2 = std::chrono::high_resolution_clock::now();
        totalTime += std::chrono::duration_cast<std::chrono::nanoseconds>(t2-t1).count();
        return est;
...
protected:
    ...
    virtual std::vector<double> MoM_precomputed(int qid, int L, int m, float gamma, const MatrixXf& distances) 
    { return {}; }
